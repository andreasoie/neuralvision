...
class EffinaNet(nn.Module):
    ...
    def __init__(
        ...
        for nb, oc in zip(
            ...
            reg_channels: Tuple[int, int] = oc, nb * 4
            cls_channels: Tuple[int, int] = oc, nb * self.num_classes

            reg_head_weight_style = "default"
            cls_head_weight_style = (
                "classification" if self.use_weightstyle else "default"
            )

            if self.use_deeper_head:
                rhead = create_subnet(
                    *reg_channels, reg_head_weight_style, nb, self.num_classes
                )
                chead = create_subnet(
                    *cls_channels, cls_head_weight_style, nb, self.num_classes
                )
            else:
                rhead = create_singlenet(
                    *reg_channels, reg_head_weight_style, nb, self.num_classes
                )
                chead = create_singlenet(
                    *cls_channels, cls_head_weight_style, nb, self.num_classes
                )
            self.regression_heads.append(rhead)
            self.classification_heads.append(chead)
    ...
    def regress_boxes(self, features):
        ...
        for i, x in enumerate(features):
            ...
            bbox_conf = self.classification_heads[i](x).view(
