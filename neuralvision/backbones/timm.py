...
if __name__ == "__main__":
    ...
    from .timstuff import get_efficientdet_config
    from timm.models.layers import set_layer_config

    # dest='torchscript', action='store_true'
    # create model
    from pprint import pprint
    from timm.models.registry import _model_entrypoints

    pprint(_model_entrypoints)

    model_type = "efficientdet-d1"

    with set_layer_config(scriptable=False):
        extra_args = dict(image_size=(128, 1024))
        bench = create_model(
            model_type,
            bench_task="train",
            num_classes=8,
