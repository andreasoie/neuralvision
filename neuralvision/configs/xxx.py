...
train.epochs = 2
NUM_CLASSES = 8  # Add 1 for background

model_name = "tf_efficientdet_d1"
pretrained = True
create_labeler = True

tmp_config = get_efficientdet_config(model_name)

tmp_model = EfficientDet(tmp_config, pretrained_backbone=False)

if pretrained:
    load_pretrained(tmp_model, tmp_config.url)

tmp_model.reset_head(num_classes=NUM_CLASSES)

model = L(DetBenchTrain)(model=tmp_model, create_labeler=create_labeler)
