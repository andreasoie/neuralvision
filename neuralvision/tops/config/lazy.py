...
class LazyCall:
    ...
    def __init__(self, target):
        if not (callable(target) or isinstance(target, (str, abc.Mapping))):
            raise TypeError(
                f"target of LazyCall must be a callable \
                  or defines a callable! Got {target}"
            )
        self._target = target
...
class LazyConfig:
    ...
    def load(filename: str, keys: Union[None, str, Tuple[str, ...]] = None):
        ...
        Args:
            filename: absolute path or relative path
    ...
    def save(cfg, filename: str):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "The config contains objects that cannot \
                                    serialize to a valid yaml. "
