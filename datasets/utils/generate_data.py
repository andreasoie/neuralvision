...
def generate_dataset(
    ...
    for image_id in tqdm.trange(
        ...
        with open(label_target_path, "w") as fp:
            ...
            for labl, bbox in zip(labels, bboxes):
                bbox = [str(_) for _ in bbox]
                to_write = f"{labl}," + ",".join(bbox) + "\n"
                fp.write(to_write)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-digits-per-image", default=20, type=int)
    args = parser.parse_args()
    X_train, Y_train, X_test, Y_test = mnist.load_download_extract_mnist()
    for dataset, (X, Y) in zip(
        ["train", "test"], [[X_train, Y_train], [X_test, Y_test]]
    ):
        num_images = (
            args.num_train_images if dataset == "train" else args.num_test_images
        )
        generate_dataset(
